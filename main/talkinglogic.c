...
void dht_sensor_task(void *pvParameter) {
    while (1) {
        ...
        if (result == ESP_OK) {
            ...
            ESP_LOGI(TAG, "🌡️ Temp: %.1f°C, 💧 Humidity: %.1f%%", temp, hum);

            if (temp < 20.0) {
                ESP_LOGI(TAG, "❄️ It's quite chilly today. Stay warm!");
            ...
            ESP_LOGE(TAG, "⚠️ Failed to read DHT22 sensor.");
        }
...
void gas_sensor_task(void *pvParameter) {
    ...
    while (1) {
        ...
        if (gas_level == 1) {
            ESP_LOGW(TAG, "⚠️ Gas Detected! Be careful!");
        } else {
